...
public class TaskerExceptionHandler {
    ...
    @ResponseStatus(HttpStatus.UNPROCESSABLE_ENTITY)
    public ResponseEntity<VndErrors> clientException(FeignClientException e, WebRequest request) {
        VndErrors error = new VndErrors(request.toString(), "problem in connecting to client : " + e.getMessage());
        ResponseEntity<VndErrors> responseEntity = new ResponseEntity<>(error, HttpStatus.UNPROCESSABLE_ENTITY);
        return responseEntity;
    }
